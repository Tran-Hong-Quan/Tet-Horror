...
public static class Utilities
{
    ...
    public static Coroutine DelayNextFrame(this MonoBehaviour mono, Action onDone)
    {
        return mono.StartCoroutine(StartDelayNextFrame(onDone));
    }

    private static IEnumerator StartDelayNextFrame(Action onDone)
    {
    ...
    public static Coroutine UpdateFunction(this MonoBehaviour mono, float duration, Action onUpdate, Action onDone = null)
    {
        return mono.StartCoroutine(StartUpdateFunction(duration, onUpdate, onDone));
    }
    private static IEnumerator StartUpdateFunction(float duration, Action onUpdate, Action onDone)
    {
        while (duration > 0)
        {
            onUpdate?.Invoke();
    ...
    public static Coroutine LoopDelay(this MonoBehaviour mono, float duration, float delay, Action onUpdate, Action onDone = null)
    {
        IEnumerator LoopDelayIEnumerator()
        {
            while (duration > 0)
            {
                onUpdate?.Invoke();
        ...
        return mono.StartCoroutine(LoopDelayIEnumerator());
    }
